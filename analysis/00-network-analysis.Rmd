...
get_centralities <- function(graph) {
  node_centralities <- graph %>% 
    ...
    mutate(
      ...
      eigen_dist = centrality_eigen(weights = distance),
      eigen_inv_dist = centrality_eigen(weights = inv_distance),
      eigen_grav_pop = centrality_eigen(weights = gravity_pop),
      eigen_modgrav_pop_ori = centrality_eigen(weights = mod_gravity_pop_ori),
      eigen_modgrav_pop_dest = centrality_eigen(weights = mod_gravity_pop_dest),
      eigen_modgrav_pop = centrality_eigen(weights = mod_gravity_pop),
      eigen_grav_adef = centrality_eigen(weights = gravity_adef),
      eigen_modgrav_adef_ori = centrality_eigen(weights = mod_gravity_adef_ori),
      eigen_modgrav_adef_dest = centrality_eigen(weights = mod_gravity_adef_dest),
      eigen_modgrav_adef = centrality_eigen(weights = mod_gravity_adef),
    ) %>% 
    as_tibble() %>% 
    mutate(across(strength_dist:eigen_modgrav_adef, rescale))
}
```
